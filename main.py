...
from pipeline.db import (
    ...
    set_company_tier, get_job, get_company,
)
...
def mark_applied(job_id, company_id):
    """Mark yourself as having applied to a job (bookkeeping only; use `apply` to fill the form)."""
    conn = init_db(DB_PATH)
    try:
...
def set_status(job_id, company_id, status):
    ...
    try:
        ...
        if updated == 0:
            raise click.UsageError(
                f"No tracked application for {job_id}. Use mark-applied first."
            )
